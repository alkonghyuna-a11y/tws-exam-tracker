...
/* ────────────────────────────────────────────────────────────
   증권투자권유대행인 모의고사 회독 관리
   문항 매핑 기준: 증투권 세부과목 구성표 (100문항)
   ──────────────────────────────────────────────────────────── */

const SECTIONS = [
  { sid: 1, code: "S01", subject: 1, name: "경기분석", start: 1, end: 4 },
  { sid: 2, code: "S02", subject: 1, name: "기본적 분석", start: 5, end: 7 },
  { sid: 3, code: "S03", subject: 1, name: "기술적 분석", start: 8, end: 10 },
  { sid: 4, code: "S04", subject: 1, name: "유가증권시장", start: 11, end: 18 },
  { sid: 5, code: "S05", subject: 1, name: "코스닥시장", start: 19, end: 21 },
  { sid: 6, code: "S06", subject: 1, name: "채권시장", start: 22, end: 28 },
  { sid: 7, code: "S07", subject: 1, name: "기타 증권시장", start: 29, end: 30 },
  { sid: 8, code: "S08", subject: 2, name: "금융상품분석·투자전략", start: 31, end: 53 },
  { sid: 9, code: "S09", subject: 2, name: "투자권유 및 고객관리", start: 54, end: 65 },
  { sid: 10, code: "S10", subject: 2, name: "직무윤리·투자자분쟁예방", start: 66, end: 80 },
  { sid: 11, code: "S11", subject: 3, name: "자본시장 관련 법규", start: 81, end: 100 },
];

const SUBJECTS = [
  { no: 1, name: "증권분석 및 증권시장", total: 30, cut: 12 },
  { no: 2, name: "금융상품 및 윤리", total: 50, cut: 20 },
  { no: 3, name: "법규 및 규정", total: 20, cut: 8 },
];
...
const storeKey = (examId) => `jtg:v1:${examId}`;
const EXAM_LIST_KEY = "jtg:v1:examlist";
        ...
        <div className="hd-row">
          <div className="brand">
            <svg className="brand-ico" viewBox="0 0 64 64" aria-hidden="true">
              <rect width="64" height="64" rx="14" fill="#9A5B0D" />
              <path d="M17 12h22l10 10v30a3 3 0 0 1-3 3H17a3 3 0 0 1-3-3V15a3 3 0 0 1 3-3z" fill="#FBFCFE" />
              <path d="M39 12l10 10h-8a2 2 0 0 1-2-2v-8z" fill="#EBD9BC" />
              <rect x="21" y="20" width="16" height="4" rx="2" fill="#9A5B0D" />
              <circle cx="24" cy="34" r="4.4" fill="#9A5B0D" />
              <circle cx="37" cy="34" r="4.4" fill="none" stroke="#D8C6A8" strokeWidth="3" />
              <circle cx="24" cy="46" r="4.4" fill="none" stroke="#D8C6A8" strokeWidth="3" />
              <circle cx="37" cy="46" r="4.4" fill="#9A5B0D" />
            ...
            <span className="brand-mark">증투권</span>
...
function StatsView({ attempts, history, sectionStats, answerKey, examLabel, onDelete }) {
  ...
  return (
    <div className="stats">
      ...
      <Block
        title={focus === "all" ? `반복 오답 상세 (${history.filter((h) => h.wrong > 0).length}문항)` : `${STATUS_META[focus].label} 상세`}
        note={
          focus === "all"
            ? "한 번이라도 틀린 문항만 모았습니다. 계속 맞힌 문항은 위에서 '안정'을 눌러 보십시오. 칸 안의 숫자는 그 회독에서 고른 번호이고, U는 모름입니다."
            : "칸 안의 숫자는 그 회독에서 고른 번호입니다. U는 모름, 초록은 정답입니다."
        }
      >
        <div className="dtl">
          {filtered
...
.tws{
  ...
  --muted:#6C7787; --navy:#9A5B0D; --navy-soft:#F8EEDC;
